...
class ShieldShip: Fighter {
    ...
    override func wasHit() {
        if shieldStrength > 0 {
            shieldStrength -= 5
            print("Shield absorbed the hit! Shield strength: \(shieldStrength)")
        } else {
            health -= 5
            print("No shield left! Health is now: \(health)")
            super.wasHit()
        }
    }
}
 
let shieldedDefender = ShieldShip()
shieldedDefender.name = "Defender"
shieldedDefender.weapon = "Cannon"

shieldedDefender.moveRight()
print("Defender position: \(shieldedDefender.position)")

shieldedDefender.fire()
print("Defender remaining fire power: \(shieldedDefender.remainingFirePower)")
shieldedDefender.wasHit()
print("Shield: \(shieldedDefender.shieldStrength), Health: \(shieldedDefender.health)")
...
//changes made above inside the class
